...
function listar(req, res) {
  const rows = db.prepare('SELECT * FROM cuentas_origen WHERE user_id=? ORDER BY created_at DESC').all(req.userId);
  res.json(rows);
}
...
function crear(req, res) {
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO cuentas_origen (id,user_id,banco,tipo_cuenta,numero_cuenta,titular) VALUES (?,?,?,?,?,?)')
    .run(id, req.userId, banco, tipo_cuenta, numero_cuenta, titular);
  const row = db.prepare('SELECT * FROM cuentas_origen WHERE id=?').get(id);
  res.status(201).json(row);
}

function eliminar(req, res) {
  const result = db.prepare('DELETE FROM cuentas_origen WHERE id=? AND user_id=?').run(req.params.id, req.userId);
