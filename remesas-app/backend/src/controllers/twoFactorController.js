...
function setup(req, res) {
  ...
  res.json({
    secreto,
    otpauth: totp.otpauthURI(secreto, user.email),
  });
}
...
function activar(req, res) {
  ...
  res.json({ ok: true, mensaje: 'Autenticación en dos pasos activada' });
}
...
function estado(req, res) {
  ...
  res.json({ activado: !!(user && user.totp_enabled) });
}

module.exports = { setup, activar, desactivar, estado };
