...
const ESTADOS = {
  no_iniciado: { label: 'Sin verificar', color: 'bg-gray-100 text-gray-600', icon: '○' },
    ...
    <div className="max-w-md mx-auto space-y-5">
      ...
      <div className="card">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-gray-500">Por transacción</span><span className="font-medium">${fmtCLP(data.resumen.limites.por_transaccion)} CLP</span></div>
          <div className="flex justify-between"><span className="text-gray-500">Diario</span><span className="font-medium">${fmtCLP(data.resumen.limites.diario)} CLP</span></div>
          <div className="flex justify-between"><span className="text-gray-500">Mensual</span><span className="font-medium">${fmtCLP(data.resumen.limites.mensual)} CLP</span></div>
        </div>
        {!verificado && (
          <div className="mt-4 p-3 bg-brand-50 rounded-xl text-sm">
            ...
            <p className="text-gray-600 mt-1">Hasta <strong>${fmtCLP(data.nivel_objetivo.diario)} CLP/día</strong> y <strong>${fmtCLP(data.nivel_objetivo.mensual)} CLP/mes</strong></p>
          </div>
        )}
      </div>
      ...
      {verificado ? (
        <div className="card text-center space-y-3">
          ...
          <p className="text-sm text-gray-500">Ya disfrutas de los límites ampliados.</p>
        ...
        <div className="card text-center space-y-3">
          ...
          <p className="text-sm text-gray-500">Esto toma unos minutos. Te avisaremos cuando esté listo. Puedes seguir usando la app.</p>
        </div>
      ) : (
        <form onSubmit={enviar} className="card space-y-4">
          <p className="font-semibold">Completa tus datos</p>
          <p className="text-xs text-gray-500">Necesitamos esta información por normativa de prevención de lavado de activos. Tus datos están protegidos.</p>
